	...
	public synchronized void handleEvent(IncumbentPerformanceChangeEvent event) 
	{
		...
		if(event.incumbentChanged())
		{
			...
			if(lastIncumbent != null)
			{
				runHistory.readLock();
				try {
				this.configTracker.addConfiguration(lastIncumbent, "Displaced Incumbent", "Performance=" + runHistory.getEmpiricalCost(lastIncumbent, runHistory.getProblemInstancesRan(lastIncumbent), cutoffTime),"Runs=" + runHistory.getTotalNumRunsOfConfig(lastIncumbent));
				} finally
				{
